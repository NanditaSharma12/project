   ...
   <?php   $id = $_GET['title']; 
      // echo "SELECT * FROM blogs_data where slug='$id'";
               ...
               $blogs_intenal = $stmt2->fetch();
                $cmnt_id = $blogs_intenal['id'];
			   $blog_comment = $conn->prepare("SELECT * FROM comment where blog_id= '$cmnt_id' order by id asc");
            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                    <ul class="navbar-nav ms-auto">
                        <li class="nav-item"><a class="nav-link <?php if($page_name ==''){ echo'active'; }else{} ?>" href="http://localhost/infotech/">Home </a></li>
                        <li class="nav-item"><a class="nav-link <?php if($page_name =='who-we-are'){ echo'active'; }else{} ?>" href="http://localhost/infotech/who-we-are">who we are  </a></li>
                        <li class="nav-item"><a class="nav-link <?php if($page_name =='our-portfolio'){ echo'active'; }else{} ?>" href="http://localhost/infotech/our-portfolio"> Portfolio</a></li>
						<li class="nav-item">
								<a class="nav-link dropdown-toggle" data-toggle="dropdown" href="http://localhost/infotech/our-services" role="button" aria-haspopup="true" aria-expanded="false">Our Services</a>
								<div class="dropdown-menu">
									<a class="dropdown-item" href="http://localhost/infotech/website-development-services">Website development</a>
								<a class="dropdown-item" href="http://localhost/infotech/mobile-app-development-services">Mobile app development</a>
								<a class="dropdown-item" href="http://localhost/infotech/ui-and-ux-design-services">UI/UX design</a>
								<a class="dropdown-item" href="http://localhost/infotech/e-commerce-website-design-and-development-services">E-commerce</a>
								<a class="dropdown-item" href="http://localhost/infotech/digital-marketing-services">Digital Marketing</a>
								<a class="dropdown-item" href="http://localhost/infotech/bussiness-consultation-services">Business consultation</a>
								<a class="dropdown-item" href="http://localhost/infotech/training-and-internship">Training and internship</a>
								</div>
						</li>
...
include('inc/queries.php');
  ...
  $msg='';
	...
	$date = time();
	$sqlcomment = $conn->prepare("INSERT INTO comment (name,email,website,comment,blog_id,date)VALUES ('$name','$email','$website','$comment','$blog_id','$date')");
    $qlcmnt = $sqlcomment->execute();
	if ($qlcmnt) {
       // $msg = "New record has been added successfully !";
		//header("location:https://www.masterinfotech.com/blog/".$id);
		abc();
     } else {
        $msg =  "Not Inserted";
     }
    ...
    <section id="blog-detail">
        ...
        <div class="container">
            <div class="row top-post">
				<div class="breadcrum"><a href="http://localhost/infotech/blog">Blog</a> / <a href="/blog/<?php echo $blogs_intenal['slug']; ?>" class="active"><?php echo $blogs_intenal['title']; ?></a> </div>
				<div class="col-md-12 top-post-left">
					<div class="post post-thumb">
						<a class="post-img" href="javascript:void(0);"><img src="http://localhost/infotech/<?php echo str_replace('../','',$blogs_intenal['image']); ?>" alt="<?php echo str_replace('-',' ',$id); ?>"  title="<?php echo str_replace('-',' ',$id); ?>" ></a>
						<div class="post-body">
							<div class="post-category">
								<a href="#">Lifestyle</a>
							</div>
							<h1 class="post-title title-lg"><a href="http://localhost/infotech/blog/<?php echo $blogs_intenal['slug']; ?>"><?php echo $blogs_intenal['title']; ?></a></h1>
							<ul class="post-meta">
								<li><a href="author.php"><?php echo $blogs_intenal['added_by']; ?></a></li>
								<li><?php $yrdata= strtotime($blogs_intenal['add_date']);
    											echo date('j-M-Y', $yrdata); ?> </li>
							</ul>
						</div>
					</div>
				</div>
								...
								echo "<p style='color:green;'>$msg</p>";
									...
									<div class="left">
										...
										<p class="comment-form-url">
											...
											<input type="hidden" name ="blog_id" value="<?php echo $cmnt_id; ?>"/>
										</p>
									</div>
									<div class="right">
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
	...
	<script src="http://localhost/infotech/assets/js/jquery.min.js"></script>
	<script src="http://localhost/infotech/assets/js/jquery.royalslider.min.js"></script>
