...
?>
   <?php   
       $cat_id = strtoupper($_GET['cat']);         $stmt_count = $conn->prepare("SELECT id FROM blogs_cat where cat ='$cat_id'");		$stmt_count->execute(); 		$blog_count = $stmt_count->fetch();		$id = $blog_count['id'];
       $stmt2 = $conn->prepare("SELECT * FROM blogs_data where cat='$id'");
       $stmt2->execute(); 
       $category_wise = $stmt2->fetchAll();
